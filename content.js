$(document).ready(function () {
    ...
    let RateMyGopherURL = "https://yelt3d.com/coding-projects/umn-rate-my-professor-chrome-firefox-extension/";
...
function collectScheduleInformation(currentProfessors, resultList) {
  let classesLinkList = findClassesOnSchedulePreviewPage(document.getElementById("schedule-courses").outerHTML)
  ...
  for (let i = 0; i < classesLinkList.length; i++) {
    ...
    chrome.storage.local.get(classesLinkList[i].split('/').pop(), function(result) {
      ...
      try {console.log(classesLinkList[i].split('/').pop(), 'Current Collect Status: ', collectClassStatus.length)
    } catch {
      console.log(classesLinkList[i].split('/').pop(), 'Current Collect Status: ', collectClassStatus)
    }

      if ((collectClassStatus == 'collectInformation') || (collectClassStatus == undefined)){
        chrome.storage.local.set({[classesLinkList[i].split('/').pop()]: 'collectInformation'}, function() {
...
function findClassesOnSchedulePreviewPage(string) {
  ...
  if (newTableParent == originalTableParent) {
    scheduleBuilderClasses.push('https://schedulebuilder.umn.edu'+matches[i].getAttribute("href"))
    };
  };
  ...
  let professorList = []
    ...
    for (let i = 0; i < classes.length; i++) {
      ...
      if (classes[i].includes('type=Internet+ID')) {
        let professorName = classes[i].split('type=Internet+ID')[1].split('target="_blank">')[1].split('</a><br>')[0].toLowerCase().replace(' ','+')
